...
class NAMLModel(BaseModel):
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        y = layers.Dropout(float(np.array(hparams.dropout).flatten()[0]))(embedded_sequences_title)
        y = layers.Conv1D(
            hparams.filter_num,
            hparams.window_size,
        ...
        y = layers.Dropout(float(np.array(hparams.dropout).flatten()[0]))(y)
    ...
    def _build_bodyencoder(self, embedding_layer):
        ...
        y = layers.Dropout(float(np.array(hparams.dropout).flatten()[0]))(embedded_sequences_body)
        y = layers.Conv1D(
            hparams.filter_num,
            hparams.window_size,
            activation=hparams.cnn_activation,
